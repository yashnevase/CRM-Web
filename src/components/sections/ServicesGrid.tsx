...
export function ServicesGrid() {
  return (
    <section className="section-space bg-canvas" id="services" aria-labelledby="services-title">
      <div className="page-shell">
        ...
        <motion.div
          ...
          className="mt-12 grid auto-rows-[290px] gap-5 md:grid-cols-2 lg:grid-cols-3"
        >
...
function ServiceCard({ service }: { service: ServiceItem }) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -5 }}
      className={cn(
        'group relative flex overflow-hidden rounded-3xl p-7 shadow-card transition-shadow duration-500 hover:shadow-glass',
    ...
    >
      {image && (
        <>
          <MediaPlaceholder
            label={`${service.title} care environment`}
      ...
      <div className="relative z-10 flex w-full flex-col items-start">
        <IconCircle
          dark={image || service.variant === 'primary'}
        ...
        <div className="mt-auto">
          ...
          <p
            className={cn(
              ...
              service.variant === 'light' ? 'text-muted' : 'text-current opacity-70',
            )}
          >
            {service.description}
          </p>
