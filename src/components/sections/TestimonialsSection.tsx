      ...
      <div className="page-shell relative">
        ...
        >
          {testimonials.map((item) => (
            <motion.div variants={fadeUp} whileHover={{ y: -4 }} key={item.name}>
              <GlassCard className="h-full p-7">
                <div className="flex items-center justify-between">
                  <Quote className="h-8 w-8 text-primary/30" />
                  <div className="flex text-accent-strong">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <Star key={index} className="h-3.5 w-3.5 fill-current" />
                    ))}
                  </div>
                </div>
                <blockquote className="mt-8 text-base font-medium leading-7">
                  “{item.quote}”
                </blockquote>
                <div className="mt-8 flex items-center gap-3">
                  <Avatar src={item.image} alt={`${item.name}, patient reviewer`} />
                  <div>
