...
export function MediaPlaceholder({ label, kind = 'care', className }: MediaPlaceholderProps) {
  ...
  return (
    <div role="img" aria-label={label} className={cn('relative flex overflow-hidden bg-gradient-to-br from-primary-soft via-surface to-accent-soft', className)}>
      ...
      <span className="relative m-auto flex h-24 w-24 items-center justify-center rounded-3xl border border-white/60 bg-white/55 text-primary shadow-glass backdrop-blur-xl"><Icon className="h-10 w-10" strokeWidth={1.5} /></span>
    </div>
  );
}
